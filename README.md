...
└────────────┬─────────────────────┬───────────────────┬─────────┘
      ...
      └─────────────┘                         └──────────────────┘
             ...
             ▼
      ┌─────────────┐
      │  Cron Job   │  (dormant user re-evaluation every N hours)
      └─────────────┘
```
...
3. **Order Placed** → Kafka publishes `order_placed` event
...
| **Scheduling** | APScheduler / cron | Periodic refresh for dormant users who don't emit events |
...
├── cron/
│   └── refresh_segments.py    # Batch job for dormant user re-evaluation
├── db/
...
```
Triggers Kafka event → async segment refresh.
...
### 6. (Optional) Run the cron job manually
...
- Segment: `dormant_user` (`days_since_last_order >= 14`)
- Experiments: `dormant_user_discount` (80% see the ₹100 off widget), `dormant_user_content_strategy` (personalized top picks)
- Cron job re-evaluates dormant users nightly so they're enrolled without needing to emit an event
